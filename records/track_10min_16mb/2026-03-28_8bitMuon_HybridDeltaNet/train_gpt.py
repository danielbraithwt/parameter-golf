...
class Hyperparameters:
    ...
    # --- 8-bit Muon ---
    muon_int8_momentum = bool(int(os.environ.get("MUON_INT8_MOMENTUM", "1")))
    muon_low_rank_k = int(os.environ.get("MUON_LOW_RANK_K", 0))  # 0 = disabled, >0 = keep top-K singular directions
    # --- Hybrid DeltaNet ---
    deltanet_enabled = bool(int(os.environ.get("DELTANET_ENABLED", "1")))
    full_attn_layers = os.environ.get("FULL_ATTN_LAYERS", "3,7,10")
    deltanet_beta_init = float(os.environ.get("DELTANET_BETA_INIT", 2.0))
...
# --- 8-bit momentum quantization helpers ---

def quantize_momentum_int8(buf: Tensor) -> tuple[Tensor, Tensor]:
    """Quantize a momentum buffer to int8 with per-row fp16 scales."""
    flat = buf.reshape(-1, buf.shape[-1]).float()
    row_max = flat.abs().amax(dim=1).clamp_min(1e-12)
    scale = (row_max / 127.0).to(torch.float16)
    q = torch.clamp(torch.round(flat / scale.float()[:, None]), -127, 127).to(torch.int8)
    return q.reshape(buf.shape), scale.reshape(buf.shape[:-1])

def dequantize_momentum_int8(q: Tensor, scale: Tensor) -> Tensor:
    """Dequantize int8 momentum back to bfloat16."""
    return (q.float() * scale.float().unsqueeze(-1)).bfloat16()

def truncate_to_low_rank(buf: Tensor, k: int) -> Tensor:
    """Keep only the top-K singular directions, zero the rest.
    Supports 2D (M, N) and 3D batched (B, M, N) tensors."""
    was_2d = buf.ndim == 2
    if was_2d:
        buf = buf.unsqueeze(0)
    B, M, N = buf.shape
    k = min(k, M, N)
    buf_f = buf.float()
    # Batched truncated SVD
    U, S, Vh = torch.linalg.svd(buf_f, full_matrices=False)  # U:(B,M,K), S:(B,K), Vh:(B,K,N)
    # Zero everything beyond top-k
    U_k = U[:, :, :k]       # (B, M, k)
    S_k = S[:, :k]          # (B, k)
    Vh_k = Vh[:, :k, :]     # (B, k, N)
    result = (U_k * S_k.unsqueeze(1)) @ Vh_k  # (B, M, N)
    if was_2d:
        result = result.squeeze(0)
    return result.to(buf.dtype)
...
class Muon(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr: float, momentum: float, backend_steps: int,
                 ...
                 int8_momentum: bool = False, low_rank_k: int = 0):
        super().__init__(
            params,
        ...
        self._built = False
        self._int8_momentum = int8_momentum
        self._low_rank_k = low_rank_k  # 0 = disabled, >0 = truncate momentum to rank K
    ...
    def _build(self):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if self._int8_momentum:
                    meta['shard_mom_q'] = torch.zeros(shard_B, *tail, device=dev, dtype=torch.int8)
                    meta['shard_mom_scale'] = torch.zeros(shard_B, *(tail[:-1]), device=dev, dtype=torch.float16)
                else:
                    meta['shard_mom'] = torch.zeros(shard_B, *tail, device=dev, dtype=torch.bfloat16)
        ...
        for group in self.param_groups:
            ...
            for i, m in enumerate(self._bank_meta):
                ...
                if sharded and self._rs_futures[i] is not None:
                    ...
                    g = m['shard']
                    if self._int8_momentum:
                        buf = dequantize_momentum_int8(m['shard_mom_q'], m['shard_mom_scale'])
                    else:
                        buf = m['shard_mom']
                else:
                    g = p.grad.bfloat16()
                    state = self.state[p]
                    if self._int8_momentum:
                        if "momentum_buffer_q" not in state:
                            state["momentum_buffer_q"] = torch.zeros_like(g, dtype=torch.int8)
                            state["momentum_buffer_scale"] = torch.zeros(
                                *g.shape[:-1], device=g.device, dtype=torch.float16)
                        buf = dequantize_momentum_int8(
                            state["momentum_buffer_q"], state["momentum_buffer_scale"])
                    else:
                ...
                # Requantize momentum if using int8
                if self._int8_momentum:
                    if sharded:
                        m['shard_mom_q'], m['shard_mom_scale'] = quantize_momentum_int8(buf)
                    else:
                        state = self.state[p]
                        state["momentum_buffer_q"], state["momentum_buffer_scale"] = quantize_momentum_int8(buf)
                if nesterov:
                    update = g.add(buf, alpha=momentum)
                else:
                    update = buf

                # Low-rank truncation: keep only top-K singular directions
                if self._low_rank_k > 0:
                    update = truncate_to_low_rank(update, self._low_rank_k)
...
class Block(nn.Module):
    def __init__(
        ...
        if layer_type == "deltanet":
            self.attn = GatedDeltaNetAttention(dim, num_heads, num_kv_heads, qk_gain_init,
                                               beta_init=deltanet_beta_init)
        else:
...
class GPT(nn.Module):
    def __init__(
        ...
        gated_attention: bool = False,
        value_residual: bool = False,
        deltanet_enabled: bool = False,
        full_attn_layers: str = "3,7,10",
        deltanet_beta_init: float = 2.0,
    ):
        super().__init__()
        self._ve_target_dim = num_kv_heads * (model_dim // num_heads)  # kv_dim for value projection
        self._deltanet_enabled = deltanet_enabled
        self._full_attn_layers = set(int(x) for x in full_attn_layers.split(",") if x.strip()) if deltanet_enabled else set()
        self._deltanet_beta_init = deltanet_beta_init
        ...
        for i in range(num_layers):
            lt = "deltanet" if (self._deltanet_enabled and i not in self._full_attn_layers) else "attention"
            self.blocks.append(Block(
                ...
                layer_type=lt,
                deltanet_beta_init=self._deltanet_beta_init,
            ))
        if rope_dims > 0:
            head_dim = model_dim // num_heads
        ...
        if xsa_last_n > 0:
            for i in range(max(0, num_layers - xsa_last_n), num_layers):
                if self.blocks[i].layer_type == "attention":
...
def eval_val_sliding(
    ...
    base_model.eval()
    compiled_logits = torch.compile(base_model.forward_logits, dynamic=False, fullgraph=not getattr(base_model, '_deltanet_enabled', False))
    with torch.inference_mode():
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        deltanet_enabled=args.deltanet_enabled,
        full_attn_layers=args.full_attn_layers,
        deltanet_beta_init=args.deltanet_beta_init,
    ).to(device).bfloat16()
    ...
    # Log DeltaNet layer assignments
    deltanet_layers = [i for i, b in enumerate(base_model.blocks) if b.layer_type == "deltanet"]
    attn_layers = [i for i, b in enumerate(base_model.blocks) if b.layer_type == "attention"]
    log0(f"deltanet:enabled={args.deltanet_enabled} deltanet_layers={deltanet_layers} attn_layers={attn_layers}")
    # No DDP -- Parallel Muon handles bank grad communication via reduce-scatter,
    # and non-bank grads are manually all-reduced before Adam steps.
    _fullgraph = not args.deltanet_enabled
    ...
    optimizer_muon = Muon(
        ...
        int8_momentum=args.muon_int8_momentum,
        low_rank_k=args.muon_low_rank_k,
    )
    log0(f"muon:int8_momentum={args.muon_int8_momentum} low_rank_k={args.muon_low_rank_k}")
    ...
    eval_model = GPT(
        ...
        deltanet_enabled=args.deltanet_enabled, full_attn_layers=args.full_attn_layers,
        deltanet_beta_init=args.deltanet_beta_init,
    ).to(device).bfloat16()
    ...
    compiled_eval = torch.compile(eval_model, dynamic=False, fullgraph=not getattr(eval_model, '_deltanet_enabled', False))
    torch.cuda.synchronize()
